<?php
	...
	abstract class CahnoryORM_Record implements Iterator, ArrayAccess
	{
		...
		final	public	function	__construct(CahnoryORM_Database $db, $name, $class)
		{
		...
		private	function	_prepareRecord()
		{
			...
			$this->_queryInsert			=	'INSERT INTO '.$this->_table.'('.trim($this->_queryInsert['columns'],',').')'
										.	"\rVALUES(".trim($this->_queryInsert['values'],',').')'
										.	"\r ON DUPLICATE KEY UPDATE ".trim($this->_queryInsert['updates'],',');
		}
		...
		public	function	getValue($name)
		{
			...
			if(isset($this->_values[$name])) {
				...
				if($name == 'friends') {
					var_dump($this->_references[$name]);
					var_dump($this->_foreign[$name]);
					var_dump($this->_local[$name]);
					var_dump(array_key_exists($name, $this->_references) && !is_array($this->_relations[$name]));
				}
				if(array_key_exists($name, $this->_references) && !is_array($this->_relations[$name])) {
					if($this->_references[$name][0] == 'record') {
						$this->_relations[$name]->setValue(
							$this->_local[$name],
							$this->getValue($this->_local[$name])
						);
					} else {
						$this->_relations[$name]->setValue(
							$this->_foreign[$name],
				...
				if('CahnoryORM_Record' != get_parent_class($this->_relations[$name])) {
					var_dump('->'.$name);
					//var_dump($name, $this->_relations[$name]);
				}
					if($name == 'friends') exit();
					echo '------';
		 ...
		 *	@access protected
		 */
		public	function	hasMany($name, $column = array(), $opt = NULL)
		{
			//	Alias usage
			if(!is_array($column)) {
			...
			if($record = $this->_db->getTable($name)) {
				if(isset($opt['refRecord'])) {
					$this->_references[$column]	=	array('record',$opt['refRecord']);
				} elseif(isset($opt['refTable'])) {
					$this->_references[$column]	=	array('table',$opt['refTable']);
				} else {
					$this->_references[$column]	=	NULL;
				}
				$this->_foreign[$column]		=	(isset($opt['foreign']) ? $opt['foreign'] : $record->_primary);
				$this->_local[$column]		=	(isset($opt['local']) ? $opt['local'] : $this->_primary);
									
				$this->_relations[$column]	=	$record;
				$this->_offsets[]			=	$column;
				
				if($this->_references[$column] !== NULL) {
					if($this->_references[$column][0] == 'record') {
						$record->hasOne($this->_references[$column][1], array(
							'foreign'	=>	$this->_foreign[$column],
							'type'		=>	'INNER JOIN'));
					}
				} else {
					//$record->{$this->_foreign[$alias]}	=	$record->{$this->_local[$alias]};
				}
		...
		protected	function	hasOne($name, $alias = array(), $opt = NULL)
		{
			...
			if($record = $this->_db->getTable($name)) {				
				...
				$this->_queryFrom			.=	' '.(isset($opt['type']) ? $opt['type'] : 'LEFT JOIN')
											.	' `'.$record->_table.'` `'.$record->_alias
